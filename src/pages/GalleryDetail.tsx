...
export default function GalleryDetail() {
  ...
  async function load() {
    ...
    // Client name
    if (g.client_id) {
      const { data: cl } = await supabase.from("wedding_clients").select("name, phone").eq("id", g.client_id).maybeSingle();
      if (cl) { setClientName(cl.name); setClientPhone(cl.phone ?? null); }
    }
...
// ── ShareModal ────────────────────────────────────────────────────────────────
function ShareModal({ gallery, clientName, clientPhone, onClose }: {
  gallery: Gallery;
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
      ...
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 480, boxShadow: "0 24px 60px rgba(0,0,0,0.2)" }}
        ...
        <div style={{ padding: "20px 24px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* WhatsApp section */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: C2.sub, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>
              Enviar por WhatsApp
            </div>
            <input
              value={phone}
            ...
            {!clientPhone && (
              <p style={{ fontSize: 11, color: C2.sub, margin: "4px 0 0" }}>
                Nenhum telefone cadastrado para este cliente. Informe acima.
              </p>
            )}
          </div>
