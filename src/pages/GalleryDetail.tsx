...
import { compressWithWatermark } from "@/utils/proofing";
...
interface Photo {
  ...
  storage_path: string | null; selecionada: boolean; created_at: string;
}
...
export default function GalleryDetail() {
  ...
  async function load() {
    ...
    await Promise.all(photoList.filter(p => p.storage_path).map(async p => {
  ...
  async function handleUpload(files: FileList) {
    ...
    for (let i = 0; i < fileArr.length; i++) {
      ...
      try {
        ...
        const { error: upErr } = await supabase.storage.from("proofing-photos").upload(path, blob, { contentType: "image/webp" });
        if (upErr) toast.error(`Erro: ${file.name}`);
        else await supabase.from("proofing_photos").insert({ gallery_id: gallery.id, nome_arquivo: file.name, storage_path: path, selecionada: false });
